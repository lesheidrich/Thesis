...
\usepackage{amsthm, url, array, float, graphicx, listings, xcolor, mathtools, multirow}
...
\begin{table}[htbp]
	...
	\begin{tabular}{|c|c|c|c|c|c|c|}
		...
		\hline
		\multirow{3}{*}{1992-01-11} & \multirow{3}{*}{Chicago} & \multirow{3}{*}{Miami} & \multirow{3}{*}{108 - 99} & 1000 & 71\% & 118 - 110 \\
		&  &  &  & 5000 & 69\% & 118 - 106 \\
		&  &  &  & 10000 & 71\% & 116 - 107 \\
		\hline
		\multirow{3}{*}{1992-01-11} & \multirow{3}{*}{Chicago} & \multirow{3}{*}{Miami} & \multirow{3}{*}{108 - 99} & 1000 & 71\% & 118 - 110 \\
...
shit i wanna do in the future with this

thigs id change in the project

project evaluation

- business logic pertaining to db control wouldn't be in controller, it would be in model to allow for better readability and modularity
- i wouldn't do a view or a rest api. while it's better for a computer science project as a whole, it took away way too much time from the original thesis
- i would spend more time on the sports analytics side to allow for better calibration of the monte carlo simulation. it's just necessary for better results.
- i'm still impressed with it's accuracy though
- i would find a better way to contruct the database to allow for validation
- i would add data validation for scraped data from a second source
- this would allow for the construction of a spider (which was unnecessary so far) this would make the scraping part more comlete cuz i could crawl
for a third source if the first two don't match and this would allow me to explore new modules and options and possibilites
- i would display the results as a report with power BI and make it into a data analysis project
- i would utilize more realistic time management to keep the project at a reasonable size

overall i'm really happy i did this because i've been wanting to do it for a long time. i am fascinated about the method and it's possibilities, and
i really feel like i learned a lot from the experience. some things worked out better than i thought, like the selenium scraper and the request
settings and the proxy rotation. 

i plan to study this further. i really am interested in reading more about its uses in fincance. it would also be amazing to utilize the method
together with deep learning. it would also be great to do an investment portfolio risk assessment with this.

architecture: all classes rely directly on previous class component instantiation. i would take this out, doesn't allow for good modularity. i would make a final 
class that uses them all, and only pack them in there

\section{Conclusion}
%In this thesis, I undertook a comprehensive exploration of web scraping techniques and Monte Carlo simulations as applied to basketball game prediction. Through the development of a web scraping tool, I collected vast amounts of basketball game data from various online sources, enabling the creation of a robust dataset for analysis and simulation.

%The implementation of the Monte Carlo simulation provided valuable insights into the potential outcomes of basketball games based on historical data. Despite the inherent limitations and challenges faced, such as the complexity of basketball metrics and the absence of detailed game information, the simulation results exhibited a degree of accuracy and correlation with historical game outcomes.

%Furthermore, the comparison of simulation results across different epoch counts highlighted the importance of considering the convergence of score values and the frequency of occurrence. By plotting probability density and violin graphs for varying epoch counts, I demonstrated the diminishing occurrence of alternative high-frequency score values with an increase in epoch count.

%Throughout this thesis, I strived to provide a thorough analysis of the methodologies employed, acknowledging both their strengths and limitations. The insights gained from this research contribute to the ongoing exploration of predictive modeling in sports analytics and pave the way for future research endeavors in this field.

%In conclusion, this thesis serves as a foundational study in the application of web scraping and Monte Carlo simulations to basketball game prediction. While there is still much to explore and refine, the findings presented here offer valuable insights and directions for further research in the realm of sports analytics and predictive modeling.
