...
They describe the website analysis portion of the process is focused on reviewing potential sources of information, learning about how they operate and store data. This allows for the formulation of the scraping or crawling strategy.

Khder distinguishes between the two strategies in his 2021 study \emph{Web Scraping or Web Crawling: State of Art, Techniques, Approaches and	Application}, where he describes web scraping as the retrieval of data from websites. Web crawlers are used for traversing a multitude of websites, the purpose of which can be for example to find new websites. Krotov et al also place the parsing of acquired data here, which involves the extracting useful data intended for further processing, while discarding the rest. 
...
The extraction process utilizes HTTP (see secton \ref{sub-http}) to communicate with the website hosting server. As stated by Khder, the process can be either manual or an automated scraper \cite{Khder}. 
...
The Monte Carlo method is described by Paolo Brandimarte in his work \emph{Handbook of Monte Carlo Simulation} \cite{brandimarte} as the utilization of a random sampling process to estimate a deterministic quantity, such as a probability or expectation of an outcome. 
...
Monte Carlo methods encompass a wide range of approaches according to Aderibigbe. He does, however identify and outline a specific pattern of four key elements they all share (see Attributes of Monte Carlo Simulations p.2~\cite{Aderibigbe}):
\begin{enumerate}
